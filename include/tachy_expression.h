...
#define TACHY_OP_TYPE_CLASS(OP_NAME, OP, OP_NAME_TRAITS) \
      ...
      { \
            ...
            static inline void apply(Res& res, const Op1& x, const Op2& y) \
            { \
                  TACHY_LOG("in scalar version");     \
                  unsigned int sz = res.size(); \
                  unsigned int i; \
                  for (i = 0; i < sz; i += arch_traits_t::stride)    \
                        (*(typename arch_traits_t::packed_t*)(&res[i])) = arch_traits_t::OP_NAME_TRAITS(x.get_packed(i), y.get_packed(i)); \
                  for ( ; i < sz; ++i ) \
                        res[i] = apply(x[i], y[i]); \
            } \
      ...
      public:
                  ...
                  _res(dynamic_cast<vector_engine<NumType>*>(cache[key]))
            {
                  if (0 == _res || 0 == _res->size())
                  {
                        ...
                        unsigned int sz1 = op1.size();
                        unsigned int sz2 = op2.size();
                        unsigned int sz = sz1 && sz2 ? std::min(sz1, sz2) : sz1 + sz2;
                        _res = new vector_engine<NumType>(sz, NumType(0));
                        cache[key] = _res;
                        OpType::apply(*_res, op1, op2);
                  }
                  else
